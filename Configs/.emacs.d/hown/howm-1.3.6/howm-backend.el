...
;;; $Id: howm-backend.el,v 1.23.2.1 2007-11-08 14:51:30 hira Exp $
;;;
...
(defvar howm-folder-dispatchers (list #'howm-folder-type))
...
(defvar howm-folder-grep-internal:pages-searcher
...
(defvar howm-view-grep-log-file nil)
...
;; will be obsolete when howm-real-grep-multi is stable
...
"
  ...
  (let ((grep-command (or (and fixed-p howm-view-fgrep-command)
    ...
    (flet ((add-opt (pred x) (when (and pred x) (setq opt (cons x opt)))))
...
;; experimental [2004-04-11]
(defun howm-real-grep-multi (str file-list
                                      &optional fixed-p force-case-fold)
  (let ((grep-command (or (and fixed-p howm-view-fgrep-command)
                          howm-view-grep-command))
        (opt (list howm-view-grep-option))
...
(defvar howm-page-dispatchers (list #'howm-page-type))
...
;;   (set (make-local-variable 'howm-buffer-page page))
  )
...
(defun howm-page-mtime:buf (page)
  (encode-time 0 0 9 1 1 1970))  ;; no meaning
...
(defvar howm-viewer-dispatchers (list #'howm-viewer-type))
...
(defun howm-item-page      (item) (howm-item-nth 0 item ""))
...
(defun howm-get-keyword-buffer (&optional dir)
  (setq dir (expand-file-name dir))
  (get-buffer-create (format howm-keyword-buffer-name-format dir)))
(defun howm-open-directory-independently (dir)
  (interactive "DDirectory: ")
  (add-to-list '*howm-independent-directories*
               (expand-file-name dir))
  (let ((default-directory dir))
    (howm-view-directory dir t))
  (howm-list-normalize)
...
(defun howm-keyword-buffer ()
  (let ((d (howm-independent-search-path)))
    (if d
        (howm-get-keyword-buffer d)
      (find-file-noselect (howm-keyword-file)))))
