...
;;; $Id: riffle.el,v 1.25.2.1 2007-11-08 14:51:29 hira Exp $
;;;
...
(defcustom howm-view-split-horizontally nil
  "If t, split window horizontally to show summary and contents.
If nil, split vertically. Otherwise, split horizontally at only first time."
  :type '(radio (const :tag "Vertically" nil)
                (const :tag "Horizontally" t)
                (const :tag "Horizontally at only first time" 1))
  :group 'howm-list-bufwin)
...
(defun riffle-summary-update (item &optional new)
  (let ((execute t))
    (when (one-window-p)
      (cond (howm-view-keep-one-window (setq execute nil))
            ((eq howm-view-split-horizontally t) (split-window-horizontally))))
    (when execute
...
(defun riffle-set-mode-maybe (mode)
