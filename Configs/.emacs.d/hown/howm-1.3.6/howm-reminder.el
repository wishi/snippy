...
;;; $Id: howm-reminder.el,v 1.58.2.2 2008-05-21 13:39:51 hira Exp $
;;;
...
(defvar howm-todo-priority-func
...
(if howm-reminder-old-format
    (progn ;; old format
      ...
      (defvar howm-reminder-font-lock-keywords
        `(
...
;;             (com-beg (match-beginning howm-reminder-regexp-command-pos))
;;             (com-end (match-end  howm-reminder-regexp-command-pos))
;;             (type-beg (match-beginning howm-reminder-regexp-type-pos))
;;             (type-end (match-end howm-reminder-regexp-type-pos))
;;             (lazy-beg (match-beginning howm-reminder-regexp-laziness-pos))
;;             (lazy-end (match-end howm-reminder-regexp-laziness-pos)))
...
;;                                           "x-+~!.@"
;;                                           "0123456789")))
;; ;;                         (read-from-minibuffer (howm-action-lock-done-prompt) )))
;;                  (c (cond ((string= s "") (progn (howm-congrats) "."))
;;                           ((string= s "x") (format ". %s"
;;                                                    howm-reminder-cancel-string))
;;                           (t nil)))
;;                  (type-or-lazy (string-match (format "^\\(%s?\\)\\([0-9]*\\)$"
...
;;                      (insert (format "%s " c))))
;;                   (type-or-lazy
;;                    (let* ((ty (match-string-no-properties 1 s))
;;                           (lz (match-string-no-properties 2 s))
;;                           (orig-ty (buffer-substring-no-properties type-beg
;;                                                                    type-end))
;;                           (orig-lz (buffer-substring-no-properties lazy-beg
;;                                                                    lazy-end))
;;                           (new-ty (if (string= ty "") orig-ty ty))
;;                           (new-lz (if (string= lz "") orig-lz lz)))
;;                      (goto-char com-beg)
;;                      (delete-region com-beg com-end)
;;                      (insert new-ty new-lz)))
;;                   (t (error "Can't understand %s" s)))
;;             (goto-char com-beg))))
    ))
...
(defun howm-list-reminder (types name)
  (let* ((r (howm-reminder-regexp types))
    ...
    (howm-view-search-folder rg folder name summarizer)
    (howm-list-exclude)
...
(defcustom howm-highlight-date-regexp-format
  (regexp-quote howm-reminder-today-format)
...
(defun howm-reminder-add-font-lock ()
  (let ((today    (howm-reminder-today 0 howm-highlight-date-regexp-format))
    ...
    (cheat-font-lock-append-keywords (howm-reminder-font-lock-keywords))
    (cheat-font-lock-append-keywords `((,today
                                        (0 howm-reminder-today-face prepend))))
    (cheat-font-lock-append-keywords `((,tomorrow
                                        (0 howm-reminder-tomorrow-face prepend))))
    ))
...
(defun howm-schedule-sort-items (items)
  (interactive)
  (howm-with-schedule-summary-format
...
(defun howm-schedule-sort-by-date ()
  (interactive)
  (howm-with-schedule-summary-format
    (howm-view-sort-general #'howm-schedule-sort-converter
                            #'howm-schedule-sort-comparer)))
...
(defun howm-reminder-search (types)
  (let* ((r (howm-reminder-regexp types))
         ...
         (folder (howm-reminder-search-path-folder))
         (raw (howm-view-search-folder-items rg folder summarizer)))
;;          (raw (howm-view-search-items rg howm-directory summarizer)))
    (howm-cl-remove-if #'howm-item-exclude-p raw)))
...
See docstring of the variable `howm-menu-reminder-separators' for details."
  (let* ((cutted (howm-cl-remove-if (lambda (item)
         ...
         (sorted (howm-sort #'howm-todo-priority-ext
                            #'howm-todo-priority-ext-gt
                            cutted)))
...
(defun howm-todo-insert-separators (item-list separators)
  (let ((is (mapcar (lambda (item) (cons (howm-todo-priority item) item))
        ...
        (sep (mapcar (lambda (pair)
                       (cons (- howm-todo-priority-schedule-top
...
(defun howm-todo-sort-by-priority ()
  (interactive)
  (howm-with-schedule-summary-format
    (howm-view-sort-general #'howm-todo-priority-ext
                            #'howm-todo-priority-ext-gt)))
...
(defvar howm-congrats-command nil)
...
(defun howm-action-lock-forward-modify-current-line (form-reg cursor-reg)
  (end-of-line)
  (when (not (re-search-backward form-reg (line-beginning-position) t))
