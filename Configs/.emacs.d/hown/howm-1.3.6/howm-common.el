...
;;; $Id: howm-common.el,v 1.72.2.1 2007-11-08 14:51:30 hira Exp $
;;;
...
(defun howm-exclude-p (filename)
  (not (howm-cl-find-if-not
        ...
        (howm-search-path))))
;; (defun howm-exclude-p (filename)
;;   (string-match howm-excluded-file-regexp filename))

;; These functions should be removed.
(defun howm-item-exclude-p (item) nil)
(defun howm-list-exclude () nil)
(defun howm-view-item-exclude-p (name-checker item) nil)

;; (defun howm-item-exclude-p (item)
;;   (howm-view-item-exclude-p #'howm-exclude-p item))
;; (defun howm-list-exclude ()
;;   (howm-view-filter-general #'howm-item-exclude-p t))
;; (defun howm-view-item-exclude-p (name-checker item)
;;   (and (not (howm-view-item-privilege item))
;;        (funcall name-checker (howm-view-item-filename item))))
